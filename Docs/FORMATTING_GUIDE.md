...
- Matching the Primary Care template exactly

**CRITICAL**: When examining runs in heading paragraphs using python-docx, ALL formatting properties should show as `None`:
- `run.font.name` should be `None` (not "Times New Roman")
- `run.font.size` should be `None` (not explicit point size)
- `run.font.color` should be `None` (not explicit RGB)
- `run.font.bold` should be `None` (not True/False)
- `run.font.italic` should be `None` (not True/False)

If any of these show actual values, it means direct formatting is overriding the style, which will cause inconsistency across sections.

**Example of the problem we solved**: The Cardiology and Infectious Diseases sections had Times New Roman fonts explicitly set on their heading runs, while Primary Care had no direct formatting. This caused all three sections to look different even though they all used the same heading styles. The solution was to remove all direct formatting from runs in heading paragraphs.
...
def remove_run_formatting(run):
    """Remove all direct formatting from a run, allowing style inheritance."""
    r_pr = run._element.rPr
    if r_pr is not None:
        # Remove the existing run properties
        run._element.remove(r_pr)
        # Add back an empty run properties element
...
sections_to_fix = ["Cardiology", "Infectious Diseases"]
current_section = "Unknown"
in_section_to_fix = False

# Remove direct formatting from headings in specific sections
for para in doc.paragraphs:
    # Track which section we're in
    if para.style.name == 'Heading 1':
        current_section = para.text.strip()
        in_section_to_fix = current_section in sections_to_fix
    
    # Remove direct formatting from headings in sections that need fixing
    if in_section_to_fix and para.style.name in ['Heading 1', 'Heading 2', 'Heading 3', 'Heading 4']:
        for run in para.runs:
            if run.text.strip():  # Only process runs with content
                remove_run_formatting(run)

# Save corrected document
doc.save('your_document_fixed.docx')
```

**To fix ALL headings in document** (not just specific sections):
```python
...
- `Cordance_Health_Insights_Bank_Primary_Care.docx`

This template has properly formatted headings with NO direct formatting applied - all formatting comes from style inheritance. The Primary Care section serves as the gold standard for formatting.

Previously used templates:
- `Cordance_Health_Insights_Bank_Cardiology.docx` (had direct formatting issues)

All new documents should copy the Primary Care template's style definitions.

## Questions?

If you're unsure about formatting:
1. Open the Primary Care section of the Insights Bank document
2. Click on a heading
3. Check the Styles panel - it should show ONLY the style name (e.g., "Heading 1")
4. If it shows additional formatting, direct formatting needs to be removed
5. Use python-docx to verify: all `run.font` properties should be `None`

---

**Last Updated**: 2025-11-23
**Template Version**: Primary Care v2.0 (fixed direct formatting issue)